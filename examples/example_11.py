...
def main():
    ...
    # The univariate GPE of each output feature will give for each point x_i a
    # specific implausibility measure.
    # With the current implausibility criterion, for each x_i we take the maximum
    # implausibility across all the output
    # features. With maxno=1, the maximum is calculated across all the output
    # features (i.e., till the last worse
    # implausibility measure). If maxno=2 --> till the previous-to-last
    # worse implausibility measure and so on.
    # With this criterion, the worse-performing emulator
    # (the output feature which is theleast well captured) will
    # dominate the entire analysis and thus determine if a point is non-implausible
    # or implausible
    ...
    # Run one iteration of HM, which is: apply the implausibility
    # criterion to detect regions of non-implausible
    ...
    # note: if we omit the filepath=<path_to_file> flag, plots will
    # be simply displayed and not saved

    # How to continue on the next wave in 5 steps
    #
    # (0) Save an exact copy of the wave. We always recommend
    # saving each wave right on completion before manipulating
    # its internal structure as you might need it later for other
    # purposes (see Appendix - A2)
    ...
    # (1) From the current non-implausible region, select points to be
    # simulated and points to be used as tests
    # for the next wave
    n_tests = 10000  # number of TEST points we want for the next wave
    # number of current NIMP points we want to simulate to augment
    # training dataset for the next wave
    n_simuls = 128
    n_avail_nimps = len(
        w0.nimp_idx
    ...
    # We now have all the necessary data to run the next wave:
    # a dataset to simulate to augment the training dataset
    # and build new emulators, and new TEST points to be evaluated
    # with the new emulators. Saving the data to files.
    np.savetxt(outfiles_dir / f"X_simul_{waveno}.txt", X_simul, fmt="%.6f")
    np.savetxt(outfiles_dir / f"X_test_{waveno}.txt", X_test, fmt="%.6f")

    w.print_stats()
    # quick check on TESTS, IMP, and NIMP sets' sizes after augmentation

    # (2) Simulate the selected points X_simul
    # (3) Add the simulated points and respective results to the training
    # dataset used in the previous wave
    # (3) Train GPEs on the new, augmented training dataset
    # (4) Start a new wave of HM, where the initial parameter space
    # to be split into non-implausible and
    # implausible regions is no more a Latin Hypercube design but is
    # now the non-implausible region obtained
    ...
    fig.tight_layout()
    plt.show()
    # TEST + SIMUL points for NEXT wave are all within NIMP space CURRENT wave

    # (A2) Loading a wave object
    # You can load a wave object by providing the same data used
    # to instantiate the wave (emulator, Itrain, cutoff,
    # maxno, mean, var). This is normally done when you need
    # to re-run the wave differently. Alternatively, you can load
    # the wave object with no arguments. This is normally done when
    # you need to examine the wave internal structure.
    # Let's try loading with no arguments.
    w = Wave()
    w.load(Path(outfiles_dir) / f"wave_{waveno}.json")
    w.print_stats()
    # notice that TESTS, IMP, and NIMP sets' sizes are the same as pre-augmentation
    ...
    # W.PV: percentage emulator variance over experimental variance at each
    # point (given as a fraction)
    ...
    # W.simul_idx: indices of W.NIMP that were selected to be
    # simulated for the next wave
    ...
    # The original test set is not stored as an attribute to save space.
    # However, this information can still be
